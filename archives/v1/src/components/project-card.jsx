        ...
        <a href={project.link} target="_blank" rel="noopener noreferrer">
            <article className={`group relative ${theme.card} rounded-xl overflow-hidden border 
                ...
                <div className="relative p-6">
                    ...
                    {/* Project highlights with improved styling and alignment */}
                    {project.highlights && (
                        <div className={`mb-6 p-4 rounded-lg ${theme.skillBox}`}>
                            <ul className={`${theme.textSecondary} space-y-3`}>
                                {project.highlights.map((highlight, index) => (
                                    <li key={index} className="flex items-start gap-3 text-sm">
                                        <ArrowUpRight className="w-4 h-4 mt-0.5 text-purple-400 flex-shrink-0" />
                                        <span className="leading-relaxed flex-1">{highlight}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
